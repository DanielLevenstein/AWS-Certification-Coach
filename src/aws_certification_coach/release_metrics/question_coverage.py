...
def measure_question_coverage(rows: Iterable[dict[str, object]]) -> dict[str, object]:
    ...
    question_categories = Counter(_question_category(row) for row in questions)
    concepts = Counter(
        concept
    ...
    return {
        ...
        "question_categories": _counter_rows(question_categories),
...
def plot_question_coverage(metrics: dict[str, object], output_path: Path) -> None:
    ...
    question_categories = _series(metrics.get("question_categories", []))
    ...
    _plot_question_categories(intent_axis, question_categories)
...
def plot_question_coverage_artifacts(metrics: dict[str, object], output_dir: Path) -> dict[str, Path]:
    ...
    outputs = {
        ...
        "question_category": output_dir / "question_intent_coverage.png",
        "certification": output_dir / "question_certification_coverage.png",
    }
    plot_domain_coverage(metrics, outputs["domain"])
    plot_question_category_coverage(metrics, outputs["question_category"])
...
def plot_question_category_coverage(metrics: dict[str, object], output_path: Path) -> None:
    import matplotlib
    ...
    _plot_question_categories(axis, question_categories)
    _add_footer(figure, metrics)
    output_path.parent.mkdir(parents=True, exist_ok=True)
    figure.savefig(output_path, dpi=220, bbox_inches="tight")
    plt.close(figure)
...
def _question_category(row: dict[str, object]) -> str:
    return _text(row.get("question_category"), "Uncategorized")
...
def _series(value: object, limit: int | None = None) -> list[tuple[str, int]]:
    if not isinstance(value, list):
        return []
    series = []
    for row in value[:limit] if limit is not None else value:
...
def _plot_question_categories(axis: object, series: list[tuple[str, int]]) -> None:
    labels = [_display_label(label) for label, _ in series][::-1]
    values = [value for _, value in series][::-1]
    axis.barh(labels, values, color="#756bb1")
    axis.set_title("Question Category", fontsize=CHART_FONT_SIZES["title"], pad=12)
