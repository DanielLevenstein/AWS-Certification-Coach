...
def plot_question_coverage(metrics: dict[str, object], output_path: Path) -> None:
    ...
    concepts = _series(metrics.get("top_concepts", []), limit=14)
    ...
    concept_axis = figure.add_subplot(grid[0, 1])
    _plot_horizontal_bars(concept_axis, concepts, "Top Concepts and Services", "Occurrences", "#8f5b2d")

    cert_axis = figure.add_subplot(grid[1, 0])
    _plot_certifications(cert_axis, certifications)

    intent_axis = figure.add_subplot(grid[1, 1])
