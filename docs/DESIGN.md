...
- `scripts/train_answer_accuracy.py`: trains the diagnostic partial-credit regressor and reports MSE/MAE.
...
- The model-evaluation suite uses `semantic_similarity` precision as the release guardrail with an 80% threshold.
- V1 defaults to the local `semantic_similarity` model; the diagnostic partial-credit regressor remains available for training analysis and compatibility.
